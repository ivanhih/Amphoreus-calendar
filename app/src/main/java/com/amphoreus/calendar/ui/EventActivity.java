...
public final class EventActivity extends Activity {
    ...
    private final List<Integer> reminderValues=new ArrayList<>(Arrays.asList(-1,0,5,10,15,30,60,1440));
    ...
    private void renderProtected() {
        ...
        hint(event.writable?"此日程含复杂重复规则、例外或参会信息。请使用系统日历编辑，以保留完整信息。":"此日历为只读，或未获得写入权限。");
        if(event.system) {
