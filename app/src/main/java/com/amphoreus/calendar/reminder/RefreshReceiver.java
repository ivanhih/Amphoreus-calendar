...
public final class RefreshReceiver extends BroadcastReceiver {
    @Override public void onReceive(Context c,Intent intent) {
        PendingResult pending=goAsync(); CalendarApp.IO.execute(()->{ try { Reminders.reschedule(c); Widgets.updateAll(c); CalendarRefreshJob.schedule(c); } finally { pending.finish(); } });
    }
}
