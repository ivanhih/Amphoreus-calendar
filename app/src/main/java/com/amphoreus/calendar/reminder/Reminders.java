...
public final class Reminders {
    ...
    private static Intent alarmIntent(Context c,String key) {
        String path=key.startsWith("l:")?key.substring(2):"s/"+key.substring(2);
        return new Intent(c,ReminderReceiver.class).setData(Uri.parse("amphoreus://reminder/"+path)).putExtra("eventKey",key);
    }
    private static String normalizeKey(String key) { return key.startsWith("l:")||key.startsWith("s:")?key:"l:"+key; }
    ...
    public static String fingerprint(CalendarEvent e) {
        return e.start+"/"+e.end+"/"+e.allDay+"/"+e.zone+"/"+e.recurrence+"/"+appReminderMinutes(e)+"/"+e.reminderArt+"/"+e.title+"/"+e.system;
    }
