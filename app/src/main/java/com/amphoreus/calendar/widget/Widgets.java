        ...
        if(agenda) {
            ...
            for(int r=0;r<6;r++) { RemoteViews row=new RemoteViews(c.getPackageName(),R.layout.widget_row);
                for(int col=0;col<7;col++) {
                    ...
                    // 今天用主题色数字标记，不再用一张写死金色的圆角底图——远程视图没法给它着色，
                    // 而卡片要跟当月主题色走（这是上一轮留下的问题）。
                    day.setTextColor(R.id.widget_day,date.equals(today)?accent:YearMonth.from(date).equals(month)?Ui.TEXT:Ui.DIM);
                    day.setTextColor(R.id.widget_dot,accent);
    ...
    public static RemoteViews renderSplit(Context c,AppWidgetManager manager,EventRepository repository,int id,Bundle options) {
        ...
        for(int r=0;r<6;r++) {
            ...
            for(int col=0;col<7;col++) {
                ...
                day.setTextColor(R.id.split_day,date.equals(today)?accent:inMonth?Ui.TEXT:Ui.DIM); day.setTextColor(R.id.split_dot,accent);
